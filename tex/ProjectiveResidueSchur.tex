...
\begin{document}
  ...
  \section{Schur transversality of the A4 locking defect}
    ...
    Schur transversality is thereby reduced to the projected A4 commutator $\sigma$, with an exhaustive
    transverse/null dichotomy.
    It is not closed here: $\sigma$ requires the Lorentzian datum $\partial_sP|_0$, which is fixed together
  ...
  \section{Status of results}
    ...
    \begin{itemize}
      ...
      \item The A4 Schur-transversality problem is reduced to the projected Lorentzian commutator
      \[
        \big[\partial_sP|_0,J_\Pi\big]\big|_{\mathrm{span}(e_+,e_-)}.
      \]
      Proposition~\ref{prop:schur-dichotomy} gives an exhaustive transverse/null dichotomy, including the
      zero-mode subcase, but does not evaluate the Lorentzian datum itself.
    \end{itemize}
    ...
    \noindent\emph{Open.}
    The magnitude $|u|$ is reduced to the Lorentzian A4 problem: define the saturated Born--Infeld
    functional $\mathcal B(s)$ along the $J_\Pi$-odd modulus and compute the chiral curvature
    \[
      \mu_\chi^2:=\partial_s^2\mathcal B(0),
    \]
    with the marginal case governed by the leading non-vanishing even coefficient of $\mathcal B(s)$.
  ...
  \section{Open deliverables}
    ...
    The remaining obstruction is therefore entirely Lorentzian and concerns the Born--Infeld saturation
    functional $\mathcal B(s)$ along the $J_\Pi$-odd modulus opened by complexification.
    By Proposition~\ref{prop:schur-dichotomy}, this deliverable is preceded by the projected
    Schur-transversality test: the same Lorentzian datum $\partial_sP|_0$ determines whether the A4
    direction is Schur-transverse or Schur-null, and only in the transverse branch does the Born--Infeld
    genus decide the generation split.
    The open deliverables are:
    \begin{enumerate}
      \item Define the Lorentzian saturation functional $\mathcal B(s)$ whose saturated admissible minima
      are selected by A4.
      \item Compute the chiral curvature
      \[
        \mu_\chi^2:=\partial_s^2\mathcal B(0).
      \]
      If $\mu_\chi^2>0$, the $J_\Pi$-equivariant branch is stable and $u=0$.
      If $\mu_\chi^2<0$, A4 produces a $J_\Pi$-conjugate saturated pair $\pm s_*$, giving a spontaneous
      chiral split $u\neq0$.
      \item In the marginal case $\mu_\chi^2=0$, determine the leading non-vanishing even coefficient
      $\lambda_{2k}$ of
      \[
        \mathcal B(s)=\mathcal B(0)+\lambda_{2k}s^{2k}+O(s^{2k+2}).
      \]
      Positive $\lambda_{2k}$ stabilises the symmetric branch; negative $\lambda_{2k}$ drives a
      higher-order spontaneous break; vanishing to all accessible orders leaves $u$ as an unfixed modulus.
    \end{enumerate}
    Deliverables~1 and~2 are discharged in the A4 companion~\cite{Beau2026bim}, where $\mathcal B(s)$ is the
    Born--Infeld determinantal saturation margin and the genus is determined electric in the Schur-transverse
    branch ($\mu_\chi^2<0$, $u\neq0$).
    The companion reduces the amplitude to the sign of the cubic Schur backreaction transported from the
    third-order tilt $\partial_s^3P|_0$ and to a boundary-versus-interior selection of the saturation problem,
    leaving that sign and the chiral-frontier normalisation $\mathcal N_A$ as the residual open data.
    These are the last conceptual locks before the explicit construction of $E_\Pi$ as the Schur
    complement~\eqref{eq:schur} with a Lorentzian $J_\Pi$-odd eliminated block $1-P(s)$.
  ...
  \section{Conclusion}
    ...
    The single remaining question is whether A4 derives the antiunitary chiral equivariance defect of the
    eliminated block, and with what magnitude, which is the next text to settle from the Born--Infeld
    saturation structure~\cite{Beau2026c}.
